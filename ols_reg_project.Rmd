...
```
```{r Cube root transformation}
addhealth$inc.cuberoot = addhealth$parentinc^(1/3)
```
Because some values for parental income are marked as 0, I apply a cube-root transformation.
...
model1 <- lm_svy_mi(indegree~inc.cuberoot, imputations)
model2 <- lm_svy_mi(indegree~inc.cuberoot+nsports+pseudoGPA, imputations)
model3 <- lm_svy_mi(indegree~inc.cuberoot+nsports+pseudoGPA+honorsociety+sex, imputations)
model4 <- lm_svy_mi(indegree~pseudoGPA, imputations)
model5 <- lm_svy_mi(indegree~pseudoGPA+honorsociety, imputations)
model6 <- lm_svy_mi(indegree~honorsociety, imputations)
model7 <- lm_svy_mi(indegree~pseudoGPA+honorsociety+inc.cuberoot, imputations)
model8 <-lm_svy_mi(indegree~pseudoGPA+honorsociety+inc.cuberoot+nsports, imputations)
screenreg(lapply(list(model1, model2, model3, model4, model5, model6), convertModel),
       caption="OLS regression models predicting number of friend nominations received", custom.coef.names = c("Intercept",
                             "parental income (cube root)",
